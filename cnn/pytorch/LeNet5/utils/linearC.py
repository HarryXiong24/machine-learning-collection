...
class linearClassify(classifier):
    ...
    def train(
        self, X, Y, reg=0.0, initStep=1.0, stopTol=1e-4, stopIter=5000, plot=None
    ):
        """
        ...
        while not done:
            ...
            for i in range(M):  # for each data point
                ...
                gradi = (
                    -Y01[i] * (1 - sigx) * twod(X1[i, :])
                    + (1 - Y01[i]) * sigx * twod(X1[i, :])
                    + reg * self.theta
                )
            ...
            if plot is not None:
                plot(self, X, Y, Jsur, J01)
            # print Jsur

            # check stopping criteria:
            it += 1
            done = (it > stopIter) or (
                (it > 1) and (abs(Jsur[-1] - Jsur[-2]) < stopTol)
            )
